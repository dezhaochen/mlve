...
import utils
from encoder_pixel_Q import make_encoder
...
class Actor(nn.Module):
    ...
    def forward(
        ...
        obs_Q, obs_likelihoods = self.encoder.entropy_bottleneck(obs)
        if detach_encoder:
            obs_Q = obs_Q.detach()

        mu, log_std = self.trunk(obs_Q).chunk(2, dim=-1)
...
class Critic(nn.Module):
    ...
    def forward(self, obs, action, detach_encoder=False):
        ...
        q1 = self.Q1(obs_Q, action)
        q2 = self.Q2(obs_Q, action)
...
class SacAeAgent(object):
    ...
    def update_critic(self, obs, action, reward, next_obs, not_done, L, step):
        ...
        current_Q1, current_Q2, obs_likelihoods = self.critic(obs, action)
        ...
        loss = critic_loss + bpp
        aux_loss = self.critic.encoder.aux_loss()

        L.log('train_critic/loss', critic_loss, step)
        L.log('train_critic/bpp', bpp, step)
        L.log('train_critic/aux', aux_loss, step)
