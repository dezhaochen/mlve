...
def transmission_exp(env, agent):
    ...
    true_current_action = action_queue[-1]  # 当前执行的动作
    ...
    while not done:
        ...
        if transmission_queue and transmission_queue[0][1] == step:
            ...
            if np.random.rand() > 1 - (1 - eta) ** K:
                true_current_action = current_action

        # 使用当前有效的动作
        action_queue.append(true_current_action)

        # 执行动作并获取新状态和奖励
        obs, reward, done, _ = env.step(true_current_action)
